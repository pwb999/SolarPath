                ...
                ScrollView {
                    VStack(spacing: 8) {
                        ...
                        SunDirectionView(
                            ...
                            sunAltitude: sunAltitude   // ← pass it in
                        )
                        .frame(height: 125)
                        .padding(.top, -14)
                ...
                ScrollView {
                        ...
                        if let sn = computeSolarTransit(
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text("Solar Noon: \(formattedHHMM(sn))")
                                    .font(.caption2)
                                    .foregroundStyle(.white.opacity(0.85))
                            }
                        }
                ...
                ScrollView {
                    VStack(spacing: 8) {
                        ...
                        (
                            ...
                            +
                            Text("""
TAP the App Title or orange refresh buttons to recalculate.

Accuracy is within a few degrees/minutes and considered suitable for field use.
...
struct SunDirectionView: View {
    ...
    var sunAltitude: Int   // used to grey the sun icon at night
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ...
                if showSunriseSetLines {
                    ...
                    // Sunrise azimuth label
                    Text("\(sunriseAzimuth)°")
                        .font(.caption2)
                        .foregroundColor(.green)
                        .opacity(0.9)
                        .position(
                            x: center.x + (radius + labelOffset * 0.75) * CGFloat(sin(riseRadians)),
                            y: center.y - (radius + labelOffset * 0.75) * CGFloat(cos(riseRadians))
                        )
                    ...
                    // Sunset azimuth label
                    Text("\(sunsetAzimuth)°")
                        .font(.caption2)
                        .foregroundColor(.red)
                        .opacity(0.9)
                        .position(
                            x: center.x + (radius + labelOffset * 0.75) * CGFloat(sin(setRadians)),
                            y: center.y - (radius + labelOffset * 0.75) * CGFloat(cos(setRadians))
                        )
                }
                ...
                // Current azimuth label + sun icon (orange by day, grey by night)
                HStack(spacing: 2) {
                    Text("\(sunAzimuth)°")
                    Image(systemName: "sun.max.fill")
                        .foregroundColor(sunAltitude >= 0 ? .orange : .gray)
                }
                .font(.caption)
